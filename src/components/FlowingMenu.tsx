...
const MenuItem: React.FC<MenuItemProps> = ({ link, text, image, title }) => {
  ...
  const repeatedMarqueeContent = React.useMemo(() => {
    return Array.from({ length: 4 }).map((_, idx) => (
      <React.Fragment key={idx}>
        <span className="text-[#060010] font-satoshi font-normal p-[1vh_1vw_0]">{text}</span>
        <div
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none bg-white translate-y-[10%]"
        ref={marqueeRef}
      >
